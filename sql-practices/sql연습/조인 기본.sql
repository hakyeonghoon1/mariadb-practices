...
where a.emp_no = b.emp_no		-- join 조건 (n-1)개
and b.to_date ='9999-01-01';	-- row 선택 조건

-- 예제2: employees 테이블 titles 테이블을 join 하여 직원의 이름과 직책을 출력하되,
-- 여성 엔지니어만 출력하세요.
...
-- 1) natural join
-- 두 테이블에 공통 칼럼이 있으면 별다른 조인 조건없이 암묵적으로 조인이 됨
-- 쓸일 없음
select a.first_name, b.title
from employees a natural join titles b -- a.emp_no = b.emp_no 생략
...
select a.title, avg(b.salary), count(*) as 인원수
...
-- 예제: 현재 부서별로 현재 직책이 Engineer인 직원들에 대해서만 평균급여를 구하세요.
select b.dept_name, c.title , avg(d.salary)
from dept_emp a, departments b, titles c, salaries d
...
-- 예제: 현재 직책별로 급여의 총합을 구하되 Engineer직책은 제외하세요
