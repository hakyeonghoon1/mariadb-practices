...
select date_format(min(hire_date),'%Y년 %m월 %d일') as 'longest'
from employees;
...
-- 문제6. 최고 어린 사원의 나이와 최 연장자의 나이는? (나이 차이)

select max(datediff(now(),birth_date)/365) -min(datediff(now(),birth_date)/365) as 나이차이
from employees
order by datediff(now(),birth_date)/365 desc;